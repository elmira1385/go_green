...
const appdevelopmwnt=document.getElementById("APPDEVELOPMENT") as HTMLDivElement
...
webDesign.addEventListener("click",()=>{
    let webDesigns=products.filter((products)=>products.description==="WEB DESIGN")
    webDesigns.forEach((element)=>{
    
    addThemEl.innerHTML+=`
    <div
          class="profiles beforeBgPro relative w-[350px] h-[233px] bg-cover bg-[url(${element.image})] flex justify-center"
        >
          <div class="massage flex flex-col justify-center items-center gap-2">
            <p class="text-white z-20 text-[18px] font-semibold">
              ${element.title}
            </p>
            <p class="text-white z-20 text-[14px] font-light">${element.description}</p>
          </div>
          <div
            class="layer2 beforeBg w-[350px] h-[233px] absolute top-0 left-0 z-30 flex justify-center items-center"
          >
